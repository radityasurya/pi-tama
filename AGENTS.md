...
Extensions publish to npm through GitHub Actions using OIDC Trusted Publishing — no local npm login, token, or publish-time 2FA is needed. Each package has a trusted publisher configured on npmjs.com that accepts publishes from `radityasurya/pi-tama` workflow `publish.yml` in the `npm` environment.
