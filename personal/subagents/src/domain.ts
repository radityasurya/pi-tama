 ...
 * types. Backends translate their native streams (pi session events) into
 * the normalized `SubagentEvent` union.
 */
...
/**
 * Shared reasoning-effort scale (pi's thinking levels). The pi backend uses
 * a value directly. Omitted = backend default (pi inherits the parent level).
 */
...
export interface SpawnTask {
  ...
  readonly cwd: string;
  /**
   * Generic model hint, interpreted by the pi backend:
   * "provider/model-id" or bare model id. Omitted = inherit parent model.
   */
  readonly model?: string;
  /** pi thinking level. Omit to inherit the parent level. */
