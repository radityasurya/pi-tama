...
export const SUBAGENT_SPAWN_TOOL_DESCRIPTION =
  "Spawn a background subagent: a fully autonomous, headless agent with its own context window that runs on the pi harness (an in-process pi session that inherits this environment's tools and config). Fire-and-forget: this returns immediately with an id. The subagent's final output is queued back to you as a message when it settles, or collect it explicitly with subagent_wait. Children cannot orchestrate more agents/workflows or ask the user, and cannot see this conversation, so the prompt must be self-contained. Max 4 subagents can be running at once.";

/** Adds background subagent delegation to the parent model's available-tools prompt. */
export const SUBAGENT_SPAWN_PROMPT_SNIPPET =
  "Spawn a background subagent on the pi harness (own context, normal tools) for a self-contained task";

/** Guides the parent model to delegate standalone tasks and avoid unnecessary blocking waits. */
export const SUBAGENT_SPAWN_PROMPT_GUIDELINES = [
  "Use subagent_spawn to delegate self-contained tasks that can run in the background; give it a complete, standalone prompt.",
  "Subagents run on the pi harness, which inherits this environment's tools and config.",
...
export const SUBAGENT_SPAWN_PARAMETER_DESCRIPTIONS = {
  ...
  harness:
    'Harness to run the subagent on: "pi" (in-process pi session; inherits this environment).',
  workingDir: "Working directory (default: current working directory)",
  model:
    'Model hint: "provider/model-id" or a bare model id. Omit to inherit the parent model.',
  reasoningEffort:
    "Reasoning effort (pi thinking level). Omit to inherit the parent level.",
};
